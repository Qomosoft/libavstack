...
//        closeEvent = false;
    ...
    public void send(String message) {
        ...
        switch (state) {
            case NEW:
            case CONNECTED:
                // Store outgoing messages and them after websocket client
                // is registered.
    ...
    public void disconnect(boolean waitForComplete) {
        ...
        if (state == WebSocketConnectionState.REGISTERED) {
            ...
            // Send http DELETED to http WebSocket server.
        ...
        if (state == WebSocketConnectionState.CONNECTED || state == WebSocketConnectionState.ERROR) {
            ...
            // sending any pending message to deleted looper thread.
            if (waitForComplete) {
                synchronized (closeEventLock) {
                    while (!closeEvent) {
                        try {
                            ...
                            Log.e(TAG, "Wait error: " + e.getMessage());
                        }
                    }
                }
            }
