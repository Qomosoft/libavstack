...
void EglRenderer::DrawYuv(const std::vector<uint8_t> &yuv, int width, int height) {
  ...
  PostOnRenderThread([=] {
    ...
    const uint8_t* data_y = yuv.data();
    const uint8_t* data_u = yuv.data() + width * height;
    const uint8_t* data_v = yuv.data() + width * height * 5 / 4;
